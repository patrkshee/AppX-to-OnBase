// Skeleton generated by Hyland Unity Editor on 6/9/2022 8:42:23 AM
// Author: Patrick Sheehan
namespace OITADMEDWSOAPGetDocumentsSchedulable
{
    using System;
	...
	public class WorkviewApp
	{
		...
		//public Hyland.Unity.WorkView.Attribute schedEnabled = null;
    ...
    public class OITADMEDWSOAPGetDocumentsSchedulable : Hyland.Unity.ISchedulableScript
    {
		...
		//Create new EDW Web Service Client
		AxServicesInterfaceSoapClient _service = new DORA_EDW_WebServices.AxServicesInterfaceSoapClient(DORA_EDW_WebServices.AxServicesInterfaceSoapClient.EndpointConfiguration.AxServicesInterfaceSoap);
        ...
        public void OnExecute(Hyland.Unity.Application app, Hyland.Unity.SchedulableEventArgs args)
        {
			...
			try
            {
				...
				for(int i=0; i<edwDocQresults.Count; i++)
				{
					...
					//Connect to EDW Web Services --------------------------------------------------------------
					
					//login and create session ticket if not already set
					config.sessionTicket = loginToEDW(app);
					
					// Get application field definitions
			        Task<DORA_EDW_WebServices.GetApplicationFieldsResponse> axFields = _service.GetApplicationFieldsAsync(config.sessionTicket, edwDoc.wvDataSource, edwDoc.wvEdwAppId);
					...
					for(int ii = 0; ii < edwQueryResults.Count; ii++)
					{
						...
						app.Diagnostics.WriteIf(Diagnostics.DiagnosticsLevel.Info, "[Doc " + ii + "] Object/Document Reference: " + docRef);
						...
						//app.Diagnostics.WriteIf(Diagnostics.DiagnosticsLevel.Info, "Document extraction time: " + edwDoc.docTime);
        ...
        public void setWorkviewVars(Hyland.Unity.Application app)
		{
			try{
				...
				//wvAppCls.schedEnabled = wvAppCls.wvSchedClass.Attributes.Find("Enabled");
        ...
        public string loginToEDW(Hyland.Unity.Application app)
		{
			try{
	                ...
	                Task<DORA_EDW_WebServices.LoginResponse> response = _service.LoginAsync(config.sessionTicket, edwDoc.wvDataSource, config.appXUser, config.appXPass, 0);
...
//				queryData.Fields.Add(new AxFieldValue(8, ""));  		// PROCESSED DATE
		        ...
		        Task<DORA_EDW_WebServices.QueryDocumentsResponse> xmlData = _service.QueryDocumentsAsync(config.sessionTicket, edwDoc.wvDataSource, SerializerHelper.ToXml(queryData), config.edwStartPosition, config.edwPageSize, config.edwMaxHits, config.edwSaveQuery, config.edwSaveAsPrivate);
				...
				app.Diagnostics.WriteIf(Diagnostics.DiagnosticsLevel.Info, "Document Count: " + axRes.Count);
        ...
        public Task<DORA_EDW_WebServices.ExportDocumentPagesByRefResponse> getJobKeyFromEDW(Hyland.Unity.Application app, string docRef)
		{
			try{
				...
				Task<DORA_EDW_WebServices.ExportDocumentPagesByRefResponse> jobKey = _service.ExportDocumentPagesByRefAsync(config.sessionTicket, docRef, SerializerHelper.ToXml(axExportData));
				...
				app.Diagnostics.WriteIf(Diagnostics.DiagnosticsLevel.Info, "Preparation job key result: " + jobKey.Result.ExportDocumentPagesByRefResult);	//Job key
        ...
        public AxStringArray checkDocFromEDW(Hyland.Unity.Application app, Task<DORA_EDW_WebServices.ExportDocumentPagesByRefResponse> jobKey)
		{
			try{
				...
				for (int i = 0; i < config.edwTimeout; i++)
				{
					...
					Task<DORA_EDW_WebServices.GetExportDocumentPagesResultResponse> exportDocPageResults = _service.GetExportDocumentPagesResultAsync(config.sessionTicket, edwDoc.wvDataSource, jobKey.Result.ExportDocumentPagesByRefResult, false);
        ...
        public MemoryStream streamDocFromEDW(Hyland.Unity.Application app, AxStringArray stringArray)
        {
			...
			try{
				...
				Task<DORA_EDW_WebServices.DownloadImageStreamResponse> downloadStreamResults = _service.DownloadImageStreamAsync(config.sessionTicket, edwDoc.wvDataSource, (string)stringArray.Items[0], SerializerHelper.ToXml(streamData));
